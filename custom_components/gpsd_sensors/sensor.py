...
async def async_setup_platform(
    ...
    try:
        gpsd_sensors = GPSDClient(host=host, port=port)
        result = gpsd_sensors.json_stream()
        for gps_data in gpsd_sensors.dict_stream(convert_datetime=True):
...
class GpsdClient(SensorEntity):
    ...
    async def async_update(self) -> None:
        ...
        self.gpsd_sensors = GPSDClient(host=self._host, port=self._port)
        for gps_data in self.gpsd_sensors.dict_stream(convert_datetime=True):
